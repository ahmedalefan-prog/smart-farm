    ...
    case 'crops':
      return (
        <div style={{ padding: '20px' }}>
              ...
              {farmData.lands.filter(l => l.currentCrop).map(land => {
                ...
                return (
                  <div key={land.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${colors.sand}` }}>
                    <span>{land.name} — {land.currentCrop} ({land.area} دونم)</span>
                    {daysLeft !== null && (
                      <span style={{ color: daysLeft <= 7 ? colors.red : colors.green, fontSize: '13px' }}>
          ...
          <div style={hintStyle}>اضغط ☰ للانتقال إلى المحاصيل الحقلية أو الخضروات</div>
        </div>
      );
    ...
    case 'livestock':
      return (
        <div style={{ padding: '20px' }}>
          ...
          <div style={hintStyle}>اضغط ☰ للانتقال إلى القطعان أو المنشآت</div>
        </div>
      );
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'grid', gap: '12px' }}>
            <div style={cardStyle(colors.lime)}>
              ...
              <div style={{ fontSize: '12px', opacity: 0.8 }}>{farmData.feedInventory.ingredients.length} مادة علفية</div>
            </div>
            <div style={cardStyle(colors.sky)}>
              <div style={{ fontSize: '13px', opacity: 0.9 }}>🌱 الأراضي المزروعة</div>
              <div style={{ fontSize: '26px', fontWeight: 'bold' }}>{plantedArea} / {totalArea} دونم</div>
            </div>
          </div>
          <div style={hintStyle}>اضغط ☰ للانتقال إلى الأعلاف أو الري أو تأهيل التربة</div>
        </div>
      );
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div style={cardStyle(colors.purple)}>
              <div style={{ fontSize: '13px', opacity: 0.9 }}>📋 السجلات اليومية</div>
              <div style={{ fontSize: '26px', fontWeight: 'bold' }}>{farmData.dailyLogs.length}</div>
              <div style={{ fontSize: '12px', opacity: 0.8 }}>إجمالي السجلات</div>
            </div>
          ...
          <div style={hintStyle}>اضغط ☰ للانتقال إلى الدورة المغلقة أو التقارير</div>
        </div>
      );
      ...
      return (
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '20px', textAlign: 'center' }}>
            ...
            <p style={{ color: colors.dark, fontWeight: 'bold', marginBottom: '8px' }}>موسوعة السلالات</p>
            <p style={{ color: colors.soil }}>19 سلالة حيوانية مع أمراض ولقاحات</p>
          </div>
          <div style={hintStyle}>اضغط ☰ للانتقال إلى الموسوعة</div>
        </div>
      );
    ...
    default:
      return (
        <div style={{ padding: '20px', textAlign: 'center' }}>
          ...
          <h2 style={{ color: colors.green, marginBottom: '8px' }}>مرحباً في المزرعة الذكية</h2>
          <p style={{ color: colors.soil }}>اختر قسماً من القائمة السفلية للبدء</p>
        </div>
      );
  }
};
